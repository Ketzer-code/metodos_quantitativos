...
Extraia a população correspondente por pais por ano. Divida os casos por
...
### 3) Suponha que uma ação caia 50% e depois sobe 50%, de quanto foi a variação total no período? dica(Componha o retorno destes dois períodos utilizando juros compostos.)
...
### 10) Analise da carteira escolhida

Comecamos escolhendo os tickers dos ativos no yahoofinance:

```{r}
ativos <- c("ITUB4.SA", "WEGE3.SA", "PRIO3.SA", "EMBR3.SA", "^BVSP")
```

As empresas sao, respectivamente: Itau, Weg, PetroRio e Embraer, com o indice ibovespa como comparativo. Em seguida, obtemos os retornos dos ultimos 5 anos:
...
Apos o tratamento, 5 meses foram perdidos, o que no total nao prejudica a totalidade da analise. Agora iremos visualizar os log retornos uttilizando o pacote highcharter:
...
prices_monthly %>%
  ...
  pivot_wider(names_from = "asset", values_from = "returns") %>%
  mutate(across(2:5, ~if_else(.x > !!sym("^BVSP"), 1, 0), .names = "{.col}_superior_ibov")) %>%
...
Analisando os histogramas, percebemos que o ativo da empresa Itau possui os retornos mais consistentes (no sentido de, menos volateis), enquanto a empresa PetroRio e Embraer apresentam mais volatilidade. Para incrementar essa visualizacao, podemos incluir a densidade de retornos:
